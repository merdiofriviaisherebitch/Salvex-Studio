...
export function PrivacyConsentCard() {
  ...
  useEffect(() => {
    // Check if user has already seen the privacy consent
    const hasSeenConsent = localStorage.getItem('privacy-consent-seen')
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed bottom-6 right-6 z-50 w-4/5 max-w-[300px] h-auto rounded-2xl shadow-[rgba(60,64,67,0.3)_0_1px_2px_0,rgba(60,64,67,0.15)_0_2px_6px_2px]"
          style={{ backgroundColor: '#e7dfc5' }}
        >
          <div className="flex flex-col items-center justify-between pt-9 px-6 pb-6 relative">
            ...
            <h5 className="text-sm font-semibold mb-2 text-left mr-auto" style={{ color: '#5e6a6d' }}>
              Your privacy is important to us
            </h5>

            <p className="w-full mb-4 text-sm text-justify" style={{ color: '#5A6B5F' }}>
